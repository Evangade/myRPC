...
namespace minirpc
{
    ...
    void TinyPBCodec::encode(std::vector<AbstractProtocol::s_ptr> &messages, TcpBuffer::s_ptr out_buffer)
    {
        for (auto &i : messages)
        {
    ...
    void TinyPBCodec::decode(std::vector<AbstractProtocol::s_ptr> &out_messages, TcpBuffer::s_ptr buffer)
    {
        while (1)
        {
    ...
    const char *TinyPBCodec::encodeTinyPB(std::shared_ptr<TinyPBProtocol> message, int &len)
    {
