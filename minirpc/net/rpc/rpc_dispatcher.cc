...
#include "minirpc/comm/error_code.h"
...
#include "minirpc/comm/run_time.h"
...
#define DELETE_RESOURCE(XX) \
    ...
    // 全局版本dispatch函数
    void RpcDispatcher::g_dispatch(AbstractProtocol::s_ptr request, AbstractProtocol::s_ptr response, TcpConnection *connection)
    {
        GetRpcDispatcher()->dispatch(request, response, connection);
    }
    // RPC底层对请求消息的分发及处理流程
    void RpcDispatcher::dispatch(AbstractProtocol::s_ptr request, AbstractProtocol::s_ptr response, TcpConnection *connection)
    {
        ...
        auto callbackFunc = [req_msg, rsp_msg, req_protocol, rsp_protocol, connection, rpc_controller, this]() mutable
        {
        ...
        RpcClosure *closure = new RpcClosure(nullptr, callbackFunc);
        google::protobuf::Closure *closure_2 = google::protobuf::NewCallback(this, &RpcDispatcher::callback_response, rsp_protocol, connection);
        service->CallMethod(method, rpc_controller, req_msg, rsp_msg, closure_2);
    }
    void RpcDispatcher::callback_response(std::shared_ptr<TinyPBProtocol> rsp_protocol, TcpConnection *connection)
    {
        std::vector<AbstractProtocol::s_ptr> replay_messages;
        replay_messages.emplace_back(rsp_protocol);
        connection->reply(replay_messages);
